...
export function useLastUsed() {
  ...
  useEffect(() => {
    const storedValue = localStorage.getItem("last_nobridge_login");
    if (storedValue) {
  ...
  useEffect(() => {
    if (lastUsed) {
      localStorage.setItem("last_nobridge_login", lastUsed);
    } else {
      localStorage.removeItem("last_nobridge_login");
    }
  }, [lastUsed]);
