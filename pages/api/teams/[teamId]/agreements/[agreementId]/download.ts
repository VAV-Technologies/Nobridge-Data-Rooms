...
export default async function handle(
  ...
  if (req.method === "POST") {
    ...
    try {
      ...
      // Check if the content is a Nobridge URL
      const isNobridgeUrl =
        agreement.content.includes("nobridge.co/view/") ||
        agreement.content.includes("www.nobridge.co/view/");
      ...
      if (isNobridgeUrl) {
        // Extract linkId from Nobridge URL
        const urlParts = agreement.content.split("/view/");
        if (urlParts.length < 2) {
          return res.status(400).json("Invalid Nobridge URL format");
        }
        ...
        if (!link || !link.document) {
          return res
            .status(404)
            .json("Document not found for the provided Nobridge URL");
        }
